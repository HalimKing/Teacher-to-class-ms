...
export default function TeacherCommunicationShow({ message }: PageProps) {
    const { flash } = usePage().props as PageProps;
    const isSenderView = (message.recipients?.length ?? 0) > 0;
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            From {message.sender?.name ?? 'Unknown'} · {message.sent_at ? new Date(message.sent_at).toLocaleString() : 'Draft'}
                        </p>
                    </div>
                    ...
                    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
                        <div className="grid gap-3 border-b border-slate-100 p-4 sm:grid-cols-3">
                            <div>
                                ...
                                <p className="mt-1 text-xl font-bold text-slate-900">{message.recipient_count}</p>
                            </div>
                            <div>
                                <p className="text-xs uppercase tracking-wide text-slate-500">Delivered</p>
                                <p className="mt-1 text-xl font-bold text-slate-900">{message.delivered_count}</p>
                            </div>
                            <div>
                                <p className="text-xs uppercase tracking-wide text-slate-500">Read</p>
                                <p className="mt-1 text-xl font-bold text-slate-900">{message.read_count}</p>
                            </div>
                        </div>
                            ...
                            <table className="min-w-full divide-y divide-slate-100 text-sm">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {message.recipients.map((recipient) => (
                                        <tr key={recipient.id}>
                                            <td className="px-4 py-2 font-medium text-slate-900">{recipient.name}</td>
                                            <td className="px-4 py-2 text-slate-600">{[recipient.department, recipient.faculty].filter(Boolean).join(' · ')}</td>
                ...
                <Link href={route('teacher.communication.inbox')} className="inline-flex text-sm text-slate-600 hover:underline">
                    Back to inbox
                </Link>
            </div>
        </AppLayout>
