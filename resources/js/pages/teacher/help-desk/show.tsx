...
const breadcrumbs = (ticketId: number): BreadcrumbItem[] => [
    ...
    { title: 'Ticket', href: `/teacher/help-desk/${ticketId}` },
];
...
export default function TeacherHelpDeskShow({ ticket }: PageProps) {
    ...
    const submitReply = (event: FormEvent) => {
        ...
        replyForm.post(route('teacher.help-desk.comment', ticket.id), {
            forceFormData: true,
    ...
    const closeTicket = () => {
        closeForm.post(route('teacher.help-desk.close', ticket.id));
    };
                    ...
                    <div className="space-y-6">
                        {ticket.status === 'resolved' && (
                            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5">
                                <p className="text-sm text-emerald-900">This ticket is marked resolved. You can close it if the issue is settled.</p>
                                <button
                                    type="button"
                                    onClick={closeTicket}
                                ...
                                {ticket.activities.map((activity) => (
                                    <div key={activity.id} className="text-sm">
                                        <p className="font-medium text-slate-800">
                                            ...
                                            {activity.to_value ? `: ${activity.to_value}` : ''}
                                        </p>
