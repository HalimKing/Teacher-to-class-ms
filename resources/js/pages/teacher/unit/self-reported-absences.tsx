...
export default function UnitSelfReportedAbsencesPage({ records, staff, filters, leadershipScope }: PageProps) {
    ...
    const handleSubmit = (event: FormEvent) => {
        ...
        router.get(route('teacher.unit.self-reported-absences.index'), {
            date_from: dateFrom,
            date_to: dateTo,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Self-reported Absences" />
            <div className="flex min-w-0 flex-col gap-6 p-3 sm:p-4 md:p-6">
                <div>
                    <h1 className="text-xl font-semibold text-sidebar-foreground sm:text-2xl">Self-reported absences</h1>
                ...
                <div className="space-y-3 md:hidden">
                    {records.length === 0 ? (
                        <div className="rounded-2xl border border-dashed border-sidebar-border px-4 py-8 text-center text-sm text-sidebar-foreground/60">
                            No self-reported absences in this range for your unit.
                        </div>
                    ) : (
                        records.map((record) => (
                            <Link
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <span className="shrink-0 text-xs text-sidebar-foreground/50">{record.date_display || record.date}</span>
                                </div>
                                <p className="mt-2 text-sm text-sidebar-foreground/75">{record.session_label}</p>
                                <p className="mt-2 line-clamp-2 text-sm text-sidebar-foreground/70">{record.reason}</p>
                ...
                <div className="hidden overflow-x-auto rounded-2xl border border-sidebar-border/60 bg-card md:block">
                        ...
                        <tbody className="divide-y divide-sidebar-border/50">
                            {records.map((record) => (
                                <tr key={`${record.kind}-${record.id}`}>
                                    <td className="px-4 py-3 text-sm">{record.date_display || record.date}</td>
                                    ...
                                    <td className="px-4 py-3 text-sm">{record.session_label}</td>
                                    <td className="max-w-xs px-4 py-3 text-sm">
                                        <p className="line-clamp-2">{record.reason}</p>
                                    </td>
                                    <td className="px-4 py-3 text-sm whitespace-nowrap">{record.submitted_at_display || '—'}</td>
