...
const CSRF_RETRY_FAILED_MESSAGE = 'Security token expired. Please refresh the page and try again.';
...
function isCsrfMismatchMessage(message: string | null | undefined): boolean {
    ...
    return normalized.includes('csrf') || normalized.includes('token mismatch') || normalized.includes('page expired');
}
...
export async function apiJsonRequest<T>(url: string, options: RequestInit = {}, retried = false): Promise<T> {
    ...
    if (!response.ok) {
        ...
        if (response.status === 401) {
            const message = (typeof payload.message === 'string' && payload.message) || 'Your session expired. Please sign in again.';
            ...
            if (typeof window !== 'undefined' && !window.location.pathname.startsWith('/login')) {
                window.setTimeout(() => {
                    ...
                    // Reload the current URL so Laravel stores it as url.intended.
