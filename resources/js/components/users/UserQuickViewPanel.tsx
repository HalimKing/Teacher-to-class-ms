...
export default function UserQuickViewPanel({ open, loading, user, data, onClose, onResetPassword }: UserQuickViewPanelProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <aside className="relative flex h-full w-full max-w-full flex-col overflow-hidden bg-white shadow-2xl sm:max-w-xl dark:bg-sidebar-accent">
                <div className="flex items-start justify-between gap-3 border-b border-sidebar-border/60 p-4 sm:p-6">
                    <div className="flex min-w-0 items-start gap-3 sm:gap-4">
                        <div className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-base font-semibold text-primary">
                            {user.initials}
                        </div>
                        <div className="min-w-0">
                            <p className="text-xs font-medium tracking-wide text-primary uppercase">Admin User</p>
                            <h2 className="mt-1 text-xl font-semibold break-words text-sidebar-foreground sm:text-2xl">{user.name}</h2>
                            <p className="break-all text-sm text-sidebar-foreground/60">{user.staff_id}</p>
                        </div>
                    </div>
                        ...
                        <div className="space-y-6">
                            <section className="rounded-xl border border-sidebar-border/60 p-4">
                                ...
                                <dl className="grid gap-3 text-sm">
                                    <div className="min-w-0">
                                        <dt className="text-sidebar-foreground/50">Email</dt>
                                        <dd className="font-medium break-all">{data.profile.email}</dd>
                                    </div>
                            ...
                            <section className="rounded-xl border border-sidebar-border/60 p-4">
                                ...
                                <div className="mt-2 flex flex-wrap gap-1">
                                    {data.permissions.permissions.map((permission) => (
                                        <span key={permission} className="max-w-full rounded bg-muted px-2 py-1 text-[11px] break-all text-sidebar-foreground/70">
                                            {permission}
                                        </span>
                                    ))}
                                </div>
                ...
                <div className="flex flex-col-reverse gap-2 border-t border-sidebar-border/60 p-4 sm:flex-row sm:flex-wrap">
                    ...
                    {can('admin.user-management.users.reset-password') && (
                        <Button type="button" className="h-10 sm:h-9" onClick={() => onResetPassword(user)}>
                            Reset Password
                        </Button>
                    )}
                </div>
