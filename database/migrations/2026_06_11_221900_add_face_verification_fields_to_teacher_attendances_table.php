...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('teacher_attendances', function (Blueprint $table) {
            if (!Schema::hasColumn('teacher_attendances', 'face_verified')) {
                $table->boolean('face_verified')->default(false)->after('status');
            }

            if (!Schema::hasColumn('teacher_attendances', 'face_match_score')) {
                $table->decimal('face_match_score', 8, 6)->nullable()->after('face_verified');
            }

            if (!Schema::hasColumn('teacher_attendances', 'face_verified_at')) {
